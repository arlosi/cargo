...
fn do_resolve(registry: &SyntheticRegistry, root: Summary) -> CargoResult<()> {
...
fn resolve_linear_chain(c: &mut Criterion) {
    ...
    for depth in [10, 50, 200] {
        let (registry, root) = build_linear_chain(depth);
        group.bench_function(format!("depth_{depth}"), |b| {
            b.iter(|| do_resolve(&registry, root.clone()).unwrap())
        });
    }
    group.finish();
}
...
fn resolve_wide_fan(c: &mut Criterion) {
    ...
    for (n_packages, n_versions) in [(10, 5), (50, 5), (100, 10)] {
        let (registry, root) = build_wide_fan(n_packages, n_versions);
...
fn resolve_diamond(c: &mut Criterion) {
    ...
    for n_diamonds in [5, 20, 50] {
        let (registry, root) = build_diamond(n_diamonds);
