...
impl<'gctx> HttpRegistry<'gctx> {
    ...
    async fn config_opt(&self) -> CargoResult<Option<RegistryConfig>> {
        ...
        if let Some(config) = &*config
            && self.is_fresh(Path::new(RegistryConfig::NAME))
        {
    ...
    async fn fetch(&self, path: &Path, index_version: Option<&str>) -> CargoResult<LoadResponse> {
        ...
        if !self.auth_required.get()
            ...
            && let Some(c) = self.config_from_filesystem()
        {
    ...
    async fn fetch_uncached_no_retry(
        ...
        if !self.fresh.borrow_mut().insert(path.to_path_buf()) {
            warn!("downloaded the index file `{}` twice", path.display());
        }
    ...
    fn tick(&self) -> CargoResult<()> {
        ...
        progress.tick(
            complete,
            complete + pending,
            &format!(" {complete} complete; {pending} pending"),
        )
    }
}
