...
impl<'a> Credential for CredentialProcessCredential {
    fn perform(
        ...
        output_from_child.read_line(&mut buffer).context("failed to read hello from credential provider")?;
        let credential_hello: CredentialHello = serde_json::from_str(&buffer).context("failed to deserialize hello")?;
        ...
        buffer.clear();
        output_from_child.read_line(&mut buffer).context("failed to read response from credential provider")?;
